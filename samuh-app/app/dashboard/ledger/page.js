...
export default async function LedgerPage() {
  ...
  // ✅ Fetch transactions without any join
  ...
  // ✅ Fetch samuh members separately for name lookup
  const { data: samuhMembers } = await supabase
    .from('samuh_members')
    .select('user_id, name')
    .eq('samuh_id', samuh.id)
  ...
  const memberMap = {}
  samuhMembers?.forEach(m => {
    if (m.user_id) memberMap[m.user_id] = m.name
  })
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-1">
          Full transaction audit trail — {samuh.name}
        </p>
      </div>
          ...
          <tbody>
            {transactions?.map(t => (
              ...
              >
                ...
                {/* ✅ Use memberMap instead of join */}
                <td className="px-5 py-3 text-gray-700">
                  {t.user_id ? (memberMap[t.user_id] || '—') : '—'}
                </td>
                ...
                <td className="px-5 py-3">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    ...
                    t.type === 'deposit_late_fee' ? 'bg-amber-100 text-amber-700'   :
            ...
            {(!transactions || transactions.length === 0) && (
              <tr>
                <td colSpan={6} className="text-center py-12 text-gray-400">
                  No transactions yet for {samuh.name}.
                </td>
              </tr>
            )}
          </tbody>
